...
gui_lua_tool = {
	type = 'GUI_INP', 	-- type identifier (as in INI file)
	channels = {},  	-- registered channels
}

lua_known_tool_types.add_type(gui_lua_tool)

gui_lua_tool.gui_params = {

	{ name = 'Torque#1 Nm:',	type = 'float',	default = '10', 	min =   '0.0', 	max = ''	},
	{ name = 'Angle#1 Deg:' ,	type = 'float', default = '1000',	min =	'0.0', 	max = ''	},
	{ name = 'Torque#2 Nm:' ,	type = 'float', default = '20', 	min = 	'0.0', 	max = ''	},
	{ name = 'Angle#2 Deg:',	type = 'float', default = '2000', 	min =   '0.0', 	max = '' 	},
	{ name = 'Torque#3 Nm:', 	type ='float',	default = '30',		min = 	'0.0', 	max = ''	},
	{ name = 'Angle#3 Deg:',	type = 'float', default = '3000', 	min = 	'0.0', 	max = ''	},
}

-------------------------------------------------------------

function gui_lua_tool.registration(tool, ini_params)

	channel = gui_lua_tool.channels[tool]
	if channel ~= nil then
...
-------------------------------------------------------------
function gui_lua_tool.poll(tool, state)
	...
	if (state == dll_tool_enable) then
		...
		local res = ShowLuaToolGUI(tool, gui_lua_tool.type, gui_lua_tool.gui_params)
		if res <= 0 then
			channel.task_state = lua_task_not_ready
			local tmp = string.format('lua_tool_request: %d %s  error code: %d', tool, gui_lua_tool.type, res)
			XTRACE(16, tmp)
		end
	end
...
-----------------------------------------------------------------------------------
function gui_lua_tool.get_conn_attr(tool)
...
function gui_lua_tool.save_results(tool, values)
...
function  gui_lua_tool.get_tool_units(tool)
...
function gui_lua_tool.process_tool_result(tool)
...
function gui_lua_tool.get_tool_result_string(tool)
	...
	return string.format('%s Nm2 %s °C', p1, p2)
end
------------------------------------------------
-- Get the tool specific footer string

function gui_lua_tool.get_footer_string(tool)
...
function gui_lua_tool.get_prg_string(tool)
...
function gui_lua_tool.process_param_list(tm, Param)
